...
pub struct GlobalSearch {
    ...
    /// Request ids of in-flight remote searches, so they can be aborted
    /// daemon-side when the query changes.
...
impl GlobalSearch {
        ...
        for root in roots {
            match root {
                ...
                LocalOrRemotePath::Remote(remote) => {
                    if FeatureFlag::RemoteGlobalSearch.is_enabled() {
                        remote_roots
                ...
                Err(err) => {
                    ...
                    me.handle_source_completed(
                        ...
                        SourceOutcome::Failure {
                            error: "Local search failed.".to_string(),
                        },
                        ctx,
                    );
                }
    ...
    fn spawn_remote_search(
        ...
        self.in_flight_remote_requests.push(request_id.clone());
        let task = ctx.spawn(
            async move { pending.result().await },
            move |me, result, ctx| {
                me.in_flight_remote_requests
                    .retain(|pending| pending != &request_id);
                me.handle_remote_search_result(search_id, &host_id, result, ctx);
            },
        );
    ...
    fn handle_remote_search_result(
        ...
        search_id: u32,
        host_id: &HostId,
        result: Result<RipgrepSearchSuccess, HostRequestError>,
        ctx: &mut ModelContext<Self>,
    ) {
        match result {
            Ok(success) => {
                let capped = success.capped;
                let items = Self::remote_matches_to_global(host_id, success);
                let match_count = items.len();
                if !items.is_empty() {
                    ctx.emit(GlobalSearchEvent::ProgressBatch { search_id, items });
                }
                self.handle_source_completed(
                    search_id,
                    SourceOutcome::Success {
                        match_count,
                        capped,
            ...
            // An abort is initiated by a newer search (or a reset), which
            // already replaced the aggregate state. Nothing to record.
            ...
            Err(err) => {
                ...
                self.handle_source_completed(
                    search_id,
                    SourceOutcome::Failure {
                        error: format!("Search failed on {host_label}."),
                    },
                    ctx,
                );
            }
    ...
    /// Records the completion of one search source. When all sources have
    ...
    fn handle_source_completed(
        ...
        match outcome {
            ...
            SourceOutcome::Failure { error } => {
                ctx.emit(GlobalSearchEvent::SourceFailed {
                    search_id,
                    error: error.clone(),
                });
                active.source_errors.push(error);
            }
        }
        ...
        if active.completed_sources == 0 {
            let error = if active.source_errors.is_empty() {
                "Global search failed.".to_string()
            } else {
                active.source_errors.join(" ")
            };
            ctx.emit(GlobalSearchEvent::Failed { search_id, error });
        } else {
            ctx.emit(GlobalSearchEvent::Completed {
                search_id,
...
/// Outcome of a single search source (the local ripgrep run, or one
/// remote host's search).
enum SourceOutcome {
    Success { match_count: usize, capped: bool },
