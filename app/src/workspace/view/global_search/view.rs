...
pub enum GlobalSearchEvent {
    ...
    /// One search source (local, or one remote host) failed. Results from
    /// the other sources remain valid.
    SourceFailed {
        search_id: u32,
        error: String,
    },
...
pub struct GlobalSearchView {
    ...
    /// Errors from individual search sources (e.g. one remote host) that
    /// did not fail the whole search.
...
impl GlobalSearchView {
    ...
    fn handle_find_model_event(&mut self, event: &GlobalSearchEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            GlobalSearchEvent::SourceFailed { search_id, error } => {
                if Some(*search_id) != self.current_search_id {
                    return;
                }

                self.source_errors.push(error.clone());
                ctx.notify();
            }
                ...
                if let Some(started_at) = self.search_started_at.take() {
                    send_telemetry_from_ctx!(
                        TelemetryEvent::GlobalSearchQueryCompleted {
                            ...
                            source_failures: self.source_errors.len(),
                        },
                        ctx
                    );
                }
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        match self.enablement {
            ...
            CodingPanelEnablementState::RemoteSession { has_remote_server } => {
                ...
                if !(has_remote_server && FeatureFlag::RemoteGlobalSearch.is_enabled()) {
                    return self.render_remote_state(app);
                }
            }
        ...
        // Per-source failures (e.g. one remote host) keep the other sources'
        // results; surface them as a notice rather than wiping the list.
        if !self.source_errors.is_empty() {
            let source_error_styles = UiComponentStyles {
                font_family_id: Some(appearance.ui_font_family()),
                font_size: Some(12.),
                font_color: Some(blended_colors::text_sub(theme, theme.background())),
                ..Default::default()
            };
            let source_error_text = Span::new(self.source_errors.join(" "), source_error_styles)
                .with_soft_wrap()
                .build()
                .finish();
            ...
            let source_error_row = Flex::row()
                .with_cross_axis_alignment(CrossAxisAlignment::Start)
                .with_child(alert_icon)
                .with_child(Shrinkable::new(1.0, source_error_text).finish())
                .finish();

            header_column = header_column.with_child(
                Container::new(source_error_row)
                    .with_margin_top(4.0)
                    .finish(),
            );
        }
