...
pub enum Harness {
    ...
    #[value(name = "codex")]
    Codex,
    /// Delegate to the `hermes` CLI for Hermes/Migi-native agent runs.
    #[value(name = "hermes", alias = "migi")]
    Hermes,
...
impl Harness {
    ...
    pub fn parse_local_child_harness(value: &str) -> Option<Self> {
        match Self::parse_orchestration_harness(value) {
            Some(harness @ (Self::Claude | Self::OpenCode | Self::Codex | Self::Hermes)) => {
                Some(harness)
            }
    ...
    pub fn from_config_name(name: &str) -> Option<Self> {
        match name {
            ...
            "hermes" | "migi" => Some(Harness::Hermes),
