...
- REST session registry and event-journal inspection:
  ...
  - WebSocket session relay adapter on the `WARP_SESSION_SHARING_SERVER_URL` seam:
    ...
    - `/sessions/join/<session_id>` validates an existing relay, returns protocol-shaped `JoinedSuccessfully` with empty/bounded v0 scrollback and preserved sharer window/source metadata, normalizes `active_prompt` to the Rust `ActivePrompt` wire shape, registers a read-only viewer socket, treats viewer reconnect initializes as `RejoinedSuccessfully` with the rejoined viewer represented in the participant list, and receives live allowlisted `OrderedTerminalEvent` fanout from the sharer with viewer-local contiguous event numbers
