...
- Self-host session-sharing registry + event journal v0:
  ...
  - WebSocket relay adapter routes on `WARP_SESSION_SHARING_SERVER_URL`:
    ...
    - `/sessions/join/<session_id>` attaches read-only viewers to an existing relay, returns protocol-shaped `JoinedSuccessfully` with empty/bounded v0 scrollback, normalizes `active_prompt` to the Rust `ActivePrompt` wire shape, preserves sharer window/source metadata, treats viewer reconnect initializes as `RejoinedSuccessfully` with the rejoined viewer in the participant list, and receives allowlisted live `OrderedTerminalEvent` fanout with viewer-local contiguous event numbers
